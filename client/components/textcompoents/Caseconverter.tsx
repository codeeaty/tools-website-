...
export default function CaseConverter() {
  ...
  const handleConvert = async (caseType: string) => {
    if (!input.trim()) {
    ...
    try {
      const response = await fetch('http://localhost:8000/text/convert', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: input, case_type: caseType }),
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <div className="mb-10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white/40 text-sm">Convert text between casing styles instantly.</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-3 space-y-5">
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
              <textarea
                ...
                className="w-full h-40 p-5 bg-transparent text-white placeholder-white/25 text-sm resize-none focus:outline-none"
              />
            ...
            <div className="rounded-2xl border border-white/10 overflow-hidden bg-[#111118]">
              <div className="p-8 flex flex-col items-center justify-center text-center min-h-[140px]">
                {loading ? (
                  ...
                  <p className="text-white text-base font-medium break-all px-4 leading-relaxed">{output}</p>
                ) : (
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 space-y-4">
              ...
              <div className="bg-white/[0.01] border border-white/5 rounded-xl p-3 text-xs text-white/40 leading-relaxed">
                Pick a style and your text converts <span className="text-white/70 font-semibold">instantly</span>. Copy the result with one click.
              </div>
            </div>
          </div>
        </div>
