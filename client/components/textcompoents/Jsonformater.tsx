...
const MODE_OPTIONS = [
  ...
  { label: "Sort Keys", value: "sort_keys" },
...
export default function JsonFormatter() {
  ...
  const handleConvert = async (mode: string) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/jsonformater', {
        method: 'POST',
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <div className="mb-10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white/40 text-sm">Format, minify, sort, and validate JSON instantly.</p>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 space-y-4">
              <p className="text-xs font-semibold text-white/35 uppercase tracking-widest">Action</p>
              ...
              <div className="bg-white/[0.01] border border-white/5 rounded-xl p-3 text-xs text-white/40 leading-relaxed">
                Paste JSON, pick an action, and get an <span className="text-white/70 font-semibold">instant</span> result. Invalid JSON shows a clear error with line and column.
              </div>
            </div>
          </div>
        </div>
